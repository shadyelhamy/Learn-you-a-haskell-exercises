...
hoSumInts x y =  higherOrderSum (\x -> x) x y
...
higherOrderSequenceApplication feach a b fbetween
	| a == b = feach a
	| otherwise = feach b `fbetween` higherOrderSequenceApplication feach a (b-1) fbetween

-- Define a factorial method using the higherOrderSequenceApplication
hoFactorial :: Int -> Int
hoFactorial a = higherOrderSequenceApplication (\x -> x) 1 a (*)
